...
export class SlackAdapter implements PlatformAdapter {
  ...
  async read(params: ReadParams): Promise<Result<ReadResult, FlnxError>> {
    ...
    if (params.threadId) {
      // Use conversations.replies for threaded messages
  ...
  // ── Upload ───────────────────────────────────────────────────────────────
  // Uses the current two-step flow (files.getUploadURLExternal + files.completeUploadExternal).
  // The legacy files.upload was deprecated May 2024 and retired March 2025.
  ...
  // ── Health Check ─────────────────────────────────────────────────────────
  // OWASP ASI04:2026 — scope validation. Warns if token is over-provisioned.
  ...
  async healthCheck(): Promise<Result<HealthStatus, FlnxError>> {
    ...
    // Check scopes from response headers (Slack returns these in x-oauth-scopes)
    // We can't get headers from our apiCall wrapper, so scope check happens at CLI level
    return Ok({
      platform: "slack",
      ok: true,
      userId: data.user_id,
  ...
  // ── Private: Normalize ───────────────────────────────────────────────────
  // All inbound messages tagged _flnx_untrusted.
  // OWASP ASI01:2026 — content is never interpreted, only transported.
  ...
  private slackError(method: string, error?: string): FlnxError {
    return {
      code: "PLATFORM_ERROR",
      platform: "slack",
      status: 200, // Slack returns 200 with error in body
